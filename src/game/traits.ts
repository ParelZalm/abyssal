...
export const TRAITS: Trait[] = [
  ...
  T({ id: 'tapetum', name: 'Tapetum Lucidum', rarity: 'rare', icon: 'eye', families: ['luminous'],
    desc: '+35% sense, and the abyss dims far less.',
    apply: g => { g.sense *= 1.35; g.eyeSize += 0.5; g.glow += 0.15; } }),

  T({ id: 'photophore', name: 'Photophores', rarity: 'rare', icon: 'glow', families: ['luminous'],
    desc: 'Bioluminescence: +30% sense, and light to hunt by.',
    apply: g => { g.sense *= 1.3; g.glow += 0.6; } }),

  T({ id: 'counterillum', name: 'Counter-Illumination', rarity: 'rare', icon: 'ghost', families: ['luminous', 'lurker'],
    desc: '+50% stealth. Predators lose you against the light.',
    apply: g => { g.stealth += 0.5; g.translucent += 0.25; g.glow += 0.2; } }),

  T({ id: 'glass', name: 'Glass Body', rarity: 'rare', icon: 'ghost', families: ['lurker'],
  ...
  T({ id: 'streamline', name: 'Fusiform Body', rarity: 'rare', icon: 'blade', families: ['sprinter'],
  ...
  T({ id: 'rete', name: 'Rete Mirabile', rarity: 'rare', icon: 'bolt', families: ['predator', 'sprinter'], minStage: 2,
    desc: '+14% speed, +25% bite. Warm muscle in cold water.',
    apply: g => { g.speed *= 1.14; g.bite *= 1.25; g.metabolism *= 1.1; } }),

  T({ id: 'algae', name: 'Symbiotic Algae', rarity: 'rare', icon: 'glow', families: ['grazer', 'luminous'],
    desc: '+2.2 health per second, −10% metabolism. Lodgers who pay rent.',
    apply: g => { g.regen += 2.2; g.glow += 0.35; g.metabolism *= 0.9; } }),

  T({ id: 'cnidocyte', name: 'Cnidocyte Graft', rarity: 'rare', icon: 'spike', families: ['grazer'],
    desc: 'Stolen stinging cells: 6% of what you eat comes back as health.',
    maxStacks: 3,
    apply: g => { g.lifesteal += 0.06; g.armor += 1; } }),

  T({ id: 'vacuum', name: 'Vacuum Feeding', rarity: 'rare', icon: 'funnel', families: ['grazer'],
  ...
  T({ id: 'rakers', name: 'Gill Rakers', rarity: 'rare', icon: 'sieve', families: ['grazer'],
  ...
  T({ id: 'pharynx', name: 'Crushing Pharynx', rarity: 'rare', icon: 'molar', families: ['predator'], minStage: 3,
  ...
  T({ id: 'anguilliform', name: 'Anguilliform Body', rarity: 'rare', icon: 'coil', families: ['lurker'], minStage: 2,
  ...
  T({ id: 'mantle', name: 'Mantle Pump', rarity: 'rare', icon: 'bell', families: ['sprinter'], minStage: 2,
    desc: 'Swim in pulses: a hard kick every 0.85 s and a long glide between, with little steady thrust.',
    maxStacks: 1,
    apply: g => { g.mantle += 1; } }),

  T({ id: 'lurk', name: 'Lie in Wait', rarity: 'rare', icon: 'crouch', families: ['lurker'], minStage: 2,
  ...
  // ---------------------------------------------------- reef organs (rare)
  T({ id: 'beak', name: 'Parrot Beak', rarity: 'common', icon: 'jaw', families: ['grazer'],
  ...
  T({ id: 'venom', name: 'Venom Barbs', rarity: 'rare', icon: 'spike', families: ['lurker'],
    desc: 'Bites leave poison: 2.5 damage a second for four seconds.',
    maxStacks: 3,
    apply: g => { g.venom += 1; g.bite *= 1.1; } }),

  T({ id: 'lure', name: 'Illicium', rarity: 'rare', icon: 'glow', families: ['luminous', 'lurker'],
  ...
  T({ id: 'claws', name: 'Pincer Claws', rarity: 'rare', icon: 'blade', families: ['predator'],
    desc: '+35% bite, and a strike holds what it hits.',
    maxStacks: 2,
    apply: g => { g.claws += 1; g.bite *= 1.35; } }),

  T({ id: 'siphon', name: 'Siphon Jet', rarity: 'rare', icon: 'funnel', families: ['sprinter'],
    desc: 'A 40% harder boost that costs 20% less to hold.',
    maxStacks: 2,
    apply: g => { g.jet += 1; g.turn *= 1.08; } }),

  T({ id: 'frill', name: 'Anemone Frill', rarity: 'rare', icon: 'spiral', families: ['lurker'],
    desc: 'A stinging fringe: attackers take recoil, and you are 15% harder to notice.',
    apply: g => { g.frill += 1; g.stealth += 0.15; } }),

  // ------------------------------------------------------------------ apex
  T({ id: 'ampullae', name: 'Ampullae of Lorenzini', rarity: 'apex', icon: 'wave', families: ['predator'], minStage: 3,
    desc: '+85% sense. Read the electric field of every heartbeat.',
    apply: g => { g.sense *= 1.85; g.eyeSize += 0.2; } }),

  T({ id: 'apexjaw', name: 'Apex Predator', rarity: 'apex', icon: 'teeth', families: ['predator'], minStage: 3,
  ...
  T({ id: 'burst', name: 'White Muscle Burst', rarity: 'apex', icon: 'bolt', families: ['sprinter'], minStage: 3,
    desc: '+34% speed, +22% turning. Terrifying acceleration.',
    apply: g => { g.speed *= 1.34; g.turn *= 1.22; g.metabolism *= 1.15; } }),

  T({ id: 'titanjaw', name: 'Titan Jaws', rarity: 'apex', icon: 'gullet', families: ['predator'], minStage: 3,
  ...
  T({ id: 'ram', name: 'Ram Ventilation', rarity: 'apex', icon: 'gill', families: ['sprinter'], minStage: 4,
    desc: '+20% speed, −30% metabolism — but you must keep moving.',
    apply: g => { g.speed *= 1.2; g.metabolism *= 0.7; g.ram += 1; } }),

  T({ id: 'neurotoxin', name: 'Neurotoxin', rarity: 'apex', icon: 'drop', families: ['lurker'], minStage: 3,
    desc: 'Venom that keeps working: 8 damage a second, and +20% bite.',
    apply: g => { g.venom += 2.2; g.bite *= 1.2; } }),

  T({ id: 'deeplantern', name: 'Deep Lantern', rarity: 'apex', icon: 'glow', families: ['luminous'], minStage: 3,
    desc: 'A lure the whole trench can see. Prey comes to you; +40% sense.',
    apply: g => { g.lure += 2; g.sense *= 1.4; g.glow += 0.7; g.gulp *= 1.2; } }),

  T({ id: 'mantis', name: 'Mantis Strike', rarity: 'apex', icon: 'bolt', families: ['predator'], minStage: 4,
